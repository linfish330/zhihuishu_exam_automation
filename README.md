...
- **两个脚本独立运行**：视频下载转写工具 (`zhihuishu_video_downloader.py`) 与考试自动答题工具 (`zhihuishu_exam_automation.py`) 相互独立，各司极职。
- **本地知识库自动生成**：下载课程视频后，自动将视频音频分离并使用语音识别 (ASR) 转写为 Markdown 格式的文稿。
- **参考资料智能检索 (RAG)**：答题时，程序会**自动递归遍历** `./reference_materials` 文件夹中的所有参考资料（包含视频转录文稿及自定义文档），并使用 TF-IDF + N-gram 算法自动匹配与题干最相关的文档上下文，大幅度提升 AI 作答的准确率。
...
为了大幅提高 AI 答题的准确率（尤其是包含课内特定细节、题库中无公开答案的考题），本项目支持**本地知识库/参考资料智能检索（RAG）**功能。

### 1. 知识库统一管理
程序的所有背景知识均由 `reference_materials/` 文件夹统一管理，支持以下两路输入：
* **自动生成的视频转写库**：运行 `python zhihuishu_video_downloader.py` 下载生成的 `reference_materials/<课程名>` 文件夹，无需任何移动操作。
* **手动导入的课件与文档**：将该课程的 PDF 导出文本、复习笔记、往年题库以 `.md` 或 `.txt` 格式放入 `reference_materials/` 目录下（可以按课程建子文件夹）。

### 2. 检索与防冲突原理
程序在启动时，会**自动递归遍历** `REFERENCE_DIR`（即 `reference_materials`）目录中的所有子文件夹。
* **支持多课程隔离**：程序将以文件相对路径（如 `reference_materials/形势与政策/1.1 导言.md`）作为文档唯一标识，完美支持不同课程下存在相同章节名（例如 `1.1 导言.md`）的情形，防止文档键值碰撞。
* **智能分词检索**：答题时，程序会提取题目题干和选项作为检索 Query，针对中文采用 N-gram 双字/三字组提取，通过 TF-IDF 算法在本地倒排索引中查找最相关的 Top K 个知识分片。
* **注入上下文**：将命中的背景知识作为 Prompt 的一部分发送给 AI。这既能有效将模型聚焦于课内正确答案，又不会因一次性载入全部文件而导致超出 Token 上下文限制或产生高额 API 账单。

---

## 运行流程

1. **第一步（准备知识库）**：用户通过 `.env` 配置课程名 and API，运行 `python zhihuishu_video_downloader.py` 下载视频并自动完成音频分离、ASR 切片转写，生成对应课程的本地 Markdown 知识库至 `reference_materials/<课程名>/`。
